...
import { GitBranch, GitMerge, AlertTriangle, ArrowUp, ArrowDown, Clock, CheckCircle2 } from 'lucide-react';
...
export function RepoFlowView() {
  ...
  const renderTreeItem = (branch: typeof branches[0], isLast: boolean) => {
    ...
    const isStale = branch.ahead === 0 && branch.behind > 3;
    const isHealthy = branch.ahead > 0 && branch.behind === 0;
      ...
      <div key={branch.name} className="flex relative">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <GitBranch className={`w-5 h-5 ${isStale ? 'text-slate-500' : 'text-blue-400'}`} />
          ...
          <div className="pl-8 flex flex-col gap-2">
            <div className="flex items-center gap-4 text-sm">
              <span className={`flex items-center gap-1 ${branch.ahead > 0 ? 'text-emerald-400' : 'text-slate-500'}`}>
                <ArrowUp className="w-4 h-4" /> {branch.ahead} commit{branch.ahead !== 1 && 's'} ahead
              </span>
              <span className={`flex items-center gap-1 ${branch.behind > 0 ? 'text-rose-400' : 'text-slate-500'}`}>
                <ArrowDown className="w-4 h-4" /> {branch.behind} commit{branch.behind !== 1 && 's'} behind
              </span>
            </div>
            ...
            {mainBranch && (
              ...
              >
                ...
                <span className="text-xs text-slate-500 ml-auto flex items-center gap-1">
                  <Clock className="w-3 h-3" /> Updated {mainBranch.lastCommitTime}
                </span>
              </div>
            )}
          ...
          <div className="p-6 flex-1 overflow-y-auto">
            ...
            <div className="space-y-4 mb-8">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-slate-200 text-sm">{selectedBranch.lastCommitTime}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400 text-sm">Commits Ahead</span>
                <span className="text-emerald-400 font-bold">{selectedBranch.ahead}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-400 text-sm">Commits Behind</span>
                <span className="text-rose-400 font-bold">{selectedBranch.behind}</span>
              </div>
              <div className="flex justify-between items-center pt-4 border-t border-slate-800/50">
                <span className="text-slate-400 text-sm">Status</span>
                <span className={`text-sm font-medium ${selectedBranch.ahead > 0 && selectedBranch.behind === 0 ? 'text-emerald-400' : 'text-amber-400'}`}>
                  {selectedBranch.ahead > 0 && selectedBranch.behind === 0 ? 'Ready To Merge' : 'Requires Review'}
                </span>
              </div>
              <div className="flex justify-between items-center">
              ...
              <div className="bg-blue-950/20 border border-blue-900/50 rounded-xl p-5 mt-auto">
                ...
                <ul className="space-y-2 mb-6">
                  <li className="text-slate-300 text-sm flex items-center gap-2">
                    ...
                    {selectedBranch.ahead} commits added
                  </li>
                  <li className="text-slate-300 text-sm flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                    {Math.max(1, selectedBranch.ahead * 2)} files modified
                  </li>
                </ul>
