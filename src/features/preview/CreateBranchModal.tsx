...
export function CreateBranchModal() {
  ...
  const { createBranchModalOpen, createBranchModalBase, setCreateBranchModal } = useLayoutStore();

  const [branchName, setBranchName] = useState('');
  const [baseBranch, setBaseBranch] = useState(createBranchModalBase || 'HEAD');
  const [selectedColorIdx, setSelectedColorIdx] = useState<number>(0);

  // Update base branch when it changes from the store
  useEffect(() => {
    if (createBranchModalBase) {
      setBaseBranch(createBranchModalBase);
    }
  }, [createBranchModalBase]);

  if (!createBranchModalOpen) return null;
